...
void manage_physics_lvl6()
{
	...
	if(player.box.x >= 104U && player.box.x <= 136U && player.box.y == PLAYER_SIZE+8U && player.state == CLIMBWALK)
	{

		player.box.y = 135U;
		switch_to_level(LEVEL4);
	}
	if(player.box.x == 8U && player.box.y == 80U &&
	...
	if(player.box.x == 72U-PLAYER_SIZE && player.box.y == 80U &&
			(player.state == CROUCHWALK || player.state == CROUCH) && player.dirX == 1 )
	{
	...
	if(player.box.x == 80U && player.box.y == 80U &&
			(player.state == CROUCHWALK || player.state == CROUCH)&& player.dirX == -1 )
	{
	...
	if(player.key != NULL && checkCollision())
	{
		...
		victory();//switch_to_world(WORLD2);
